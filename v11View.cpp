...
BEGIN_MESSAGE_MAP(Cv11View, CView)
	...
	ON_WM_RBUTTONUP()
	ON_WM_LBUTTONDOWN()
	ON_COMMAND(ID_SHAPE, &Cv11View::ShapeButton)
	ON_COMMAND(ID_COLOR, &Cv11View::ColorButton)
...
void Cv11View::OnDraw(CDC* pDC)
{
	...
	CPen* lastPen = pDC->SelectObject(&pen);
	if (shape == 0) {
		pDC->Rectangle(&rc);
	}
	else if (shape == 1) {
...
LRESULT Cv11View::OnHighlightRibbonListItem(WPARAM wp, LPARAM lp)
{
	...
	CMFCRibbonColorButton* boja = (CMFCRibbonColorButton*)lp;
	...
	if (id == ID_SHAPE) {
		...
		if (index == -1)
			...
			Color = boja->GetHighlightedColor();
	}
	Invalidate();
	return 0;
}