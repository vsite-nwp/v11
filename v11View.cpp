...
Cv11View::Cv11View() :  color(0), prevColor(0), shape(0), prevShape(0){}
...
void Cv11View::OnDraw(CDC* pDC)
{
	...
	switch (shape) {
	case 0: pDC->Rectangle(rc);
		break;
	case 1: pDC->Ellipse(rc);
		break;
	case 2: pDC->RoundRect(rc, CPoint(10,10));
		break;
...
void Cv11View::OnShape()
{
	...
	CMFCRibbonGallery* galerija = (CMFCRibbonGallery*)arr.GetAt(0);
	shape = galerija->GetSelectedItem();
	prevShape = shape;
	Invalidate();
}
...
void Cv11View::OnColor()
{
	...
	CMFCRibbonColorButton* colorB = (CMFCRibbonColorButton*)arr.GetAt(0);
...
LRESULT Cv11View::OnHighlightRibbonListItem(WPARAM wp, LPARAM lp) {
	...
	UINT id = pElem->GetID();
	CMFCRibbonColorButton* colorB = (CMFCRibbonColorButton*)pElem;
	...
	case ID_COLOR:
		if (index>=0)
			color = colorB->GetHighlightedColor();
